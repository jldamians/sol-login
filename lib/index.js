...
class LoginSOL {
  ...
  // Methods
    ...
    async signin(RUC=null, username=null, password=null) {
      ...
      if (!CheckRuc.isOk(this._RUC)) {
        throw new LoginSOLError(`El número de RUC ingresado es incorrecto: ${RUC}`, {
          reprocess: false 
        });
      }
      ...
      } else if (this._page.url().indexOf('/MenuInternet.htm') !== -1 && this._page.url().indexOf('?pestana=') === -1) {
        try {
