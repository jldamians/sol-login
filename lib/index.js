  ...
  // Methods
    ...
    async signin(RUC=null, username=null, password=null) {
      ...
      try {
        ...
        throw new Error(`La opción para autenticar con RUC no está disponible: ${message}`);
      }
      ...
      if (this._page.url().indexOf('/AutenticaMenuInternet.htm') !== -1) {
        ...
        const screenshot = await HTMLElement.screenshot();

        throw new LoginSOLError(`La autenticación al portal SOL ha sido incorrecta`, { screenshot });
      }
    }
    ...
    async _setRecaptchaToken() {
       ...
       */
      const getCaptchaID = async (sitekey=null, apikey=this._2captcha.key) => {
        const params = {
       ...
       * @param {String} apikey, api key del servicio 2captcha
       */
      const getToken = async (request=null, apikey=this._2captcha.key) => {
        const task = async () => {
          const params = {
            key: apikey,
            action: 'get',
      ...
      this._2captcha.id = await getCaptchaID(this._recaptcha.sitekey);

      this._2captcha.token = await getToken(this._2captcha.id);

      await this._page.evaluate(`document.getElementById("g-recaptcha-response").innerHTML="${this._2captcha.token}";`);
    }
