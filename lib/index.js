  ...
  // Methods
    ...
    async signin(RUC=null, username=null, password=null) {
      ...
      if (this._page.url().indexOf('/AutenticaMenuInternet.htm') !== -1) {
        ...
        const warningMessage = await this._page.evaluate(function() {
          const message = document.querySelector('div.alert.alert-danger.text-left').innerText;
        ...
        if (warningMessage.indexOf('Captcha') !== -1) {
          throw new LoginSOLError(
            warningMessage, { screenshot, reprocess: true }
          );
        } else {
          throw new LoginSOLError(
            warningMessage, { screenshot, reprocess: false }
          );
        }
      }
    }
